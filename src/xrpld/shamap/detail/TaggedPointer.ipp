...
{
    XRPL_ASSERT("ripple::numAllocatedChildren : valid input", n <= SHAMapInnerNode::branchFactor);
...
{
    XRPL_ASSERT("ripple::boundariesIndex : valid input", numChildren <= SHAMapInnerNode::branchFactor);
    return std::distance(
        boundaries.begin(),
...
{
    XRPL_ASSERT("ripple::deallocateArrays : valid inputs", isFromArrayFuns[boundaryIndex](p));
...
inline TaggedPointer::TaggedPointer(RawAllocateTag, std::uint8_t numChildren)
{
    ...
    XRPL_ASSERT("ripple::TaggedPointer::TaggedPointer(RawAllocateTag, std::uint8_t) : maximum tag", tag < boundaries.size());
    XRPL_ASSERT("ripple::TaggedPointer::TaggedPointer(RawAllocateTag, std::uint8_t) : valid pointer",
    ...
    std::uint8_t toAllocate)
{
    ...
    else
    {
        ...
        XRPL_ASSERT("ripple::TaggedPointer::TaggedPointer(TaggedPointer&& ...) : non-sparse or valid sparse", !dstIsDense || dstIndex == dstNumAllocated);
